...
func handle() error {
	...
	if err := cmd.Debug().Dir(rootDir).Run("go", "run", "./script/build"); err != nil {
		return fmt.Errorf("build failed: %w", err)
	}

	fmt.Println("==> Installing disk-usage-analyser")
	if err := cmd.Debug().Dir(rootDir).Run("go", "install", "."); err != nil {
		return fmt.Errorf("go install disk-usage-analyser failed: %w", err)
	}

	fmt.Println("disk-usage-analyser installed successfully")
	return nil
}
