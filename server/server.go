...
func EnsureFrontendDevServer(ctx context.Context) (chan struct{}, error) {
	...
	cmd.Dir = "disk-usage-analyser-react/"
...
func Static(mux *http.ServeMux, opts StaticOptions) error {
	...
	reactFileSystem, err := fs.Sub(distFS, "disk-usage-analyser-react/dist")
	if err != nil {
	...
	mux.Handle("/disk-usage-analyser.svg", &mimeTypeHandler{http.FileServer(http.FS(reactFileSystem))})
...
func RegisterAPI(mux *http.ServeMux) error {
	...
	mux.HandleFunc("/api/open-iterm2", HandleOpenIterm2)
