...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if resp.Result.Repos != 0 {
		t.Fatalf("expected 0 repos, got %d", resp.Result.Repos)
	}
	if len(resp.Result.NamedHits) != 0 {
		t.Fatalf("expected 0 named hits, got %d", len(resp.Result.NamedHits))
	}
	if len(resp.Result.Binaries) != 0 {
		t.Fatalf("expected 0 binary hits, got %d", len(resp.Result.Binaries))
	}
