...
- No error is returned (must not treat `-h` as an unknown command).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if resp.Err != nil {
		t.Fatalf("tmp-files -h must not error (got: %v)\nstdout=%q stderr=%q", resp.Err, resp.Stdout, resp.Stderr)
	}
	if resp.ExitCode != 0 {
		t.Fatalf("expected exit code 0 for parent -h, got %d", resp.ExitCode)
	}
	if resp.Result != nil {
		t.Fatalf("parent -h must not run a scan; got Result=%+v", resp.Result)
	}
	out := resp.Stdout
	if strings.Contains(strings.ToLower(out), "unknown") {
		t.Fatalf("parent -h must not report unknown command:\n%s", out)
	}
	for _, want := range []string{
		...
		if !strings.Contains(out, want) {
			t.Fatalf("parent -h output missing %q:\n%s", want, out)
		}
	}
	if strings.Contains(out, "Found ") && strings.Contains(out, "binaries") {
		t.Fatalf("parent -h must not emit a scan summary:\n%s", out)
	}
