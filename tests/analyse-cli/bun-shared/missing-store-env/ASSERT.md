...
- `node_modules` row `BunSharedSize` is 0 (cache path unavailable).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if nm.BunSharedSize != 0 {
		t.Fatalf("node_modules bun_shared: expected 0 without cache, got %d", nm.BunSharedSize)
	}
	if result.Summary.BunSharedSize != 0 {
		t.Fatalf("summary bun_shared: expected 0 without cache, got %d", result.Summary.BunSharedSize)
	}
}
```