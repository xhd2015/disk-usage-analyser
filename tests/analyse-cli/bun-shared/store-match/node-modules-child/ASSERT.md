...
- `node_modules` row `BunSharedSize` is 4096 (one unique matching clone key).
- `node_modules` row `Size` is 4096.
- Summary `BunSharedSize` is 4096 (aligned with immediate-child rows).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if nm.BunSharedSize != file4K {
		t.Fatalf("node_modules bun_shared: expected %d, got %d", file4K, nm.BunSharedSize)
	}
