...
- `node_modules` row `PnpmSharedSize` is 0 (store path unavailable).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if nm.PnpmSharedSize != 0 {
		t.Fatalf("node_modules pnpm_shared: expected 0 without store, got %d", nm.PnpmSharedSize)
	}
	if result.Summary.PnpmSharedSize != 0 {
		t.Fatalf("summary pnpm_shared: expected 0 without store, got %d", result.Summary.PnpmSharedSize)
	}
}
```