...
- SSE JSON includes `hasPackageJson: true`.
- `packageManager` is `npm` (`package-lock.json` wins over `packageManager` field).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !hasPkg {
		t.Fatalf("hasPackageJson = false, want true")
	}
	if resp.NamedJSON[0].PackageManager != "npm" {
		t.Fatalf("packageManager = %q, want npm (lockfile priority)", resp.NamedJSON[0].PackageManager)
	}
}
```