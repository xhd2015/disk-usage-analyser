...
- `packageManager` is `pnpm` (parsed from Corepack `packageManager` field).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if resp.NamedJSON[0].PackageManager != "pnpm" {
		t.Fatalf("packageManager = %q, want pnpm", resp.NamedJSON[0].PackageManager)
	}
}
```