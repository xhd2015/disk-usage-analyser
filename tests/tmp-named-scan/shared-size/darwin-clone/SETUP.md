...
**Leaf**: darwin APFS store+cache clones contribute `sharedSize=4096` on `named_enriched`
...
4. `cp -c` store file into `node_modules/pnpm-pkg/file` (cache env configured; no bun clone in v1 fixture).
