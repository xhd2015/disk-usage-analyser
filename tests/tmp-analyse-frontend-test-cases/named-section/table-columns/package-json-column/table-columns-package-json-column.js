...
if (header) {
    const headerText = await page.$eval('[data-testid="node-modules-column-header"]', el => (el.textContent || '').trim()).catch(() => '');
    console.log(`HEADER_TEXT: "${headerText}"`);
    console.log(`PKGJSON_HEADER: ${headerText.includes('package.json') ? 'ok' : 'fail'}`);
    if (!headerText.includes('package.json')) process.exitCode = 1;
}

const pkgjsonCells = await page.$$('[data-testid^="node-modules-pkgjson-"]');
console.log(`COUNT node-modules-pkgjson: ${pkgjsonCells.length}`);
console.log(`ELEM node-modules-pkgjson: ${pkgjsonCells.length > 0 ? 'present' : 'MISSING'}`);

if (namedRows.length > 0 && pkgjsonCells.length === 0) {
    process.exitCode = 1;
}
...
if (namedRows.length > 0) {
    if (!header || pkgjsonCells.length === 0) process.exitCode = 1;
}
console.log('DONE');